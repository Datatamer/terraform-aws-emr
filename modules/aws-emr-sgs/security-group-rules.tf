...
//Combined TCP, UDP, ICMP SG rule sourcing EMR Managed Agent SG in EMR Managed Master SG
resource "aws_security_group_rule" "all_ports_for_agent_sg_in_master_sg" {
  from_port = 0
  to_port = 0
  protocol = "-1"
  security_group_id = aws_security_group.emr_managed_master.id
  type = "ingress"
  source_security_group_id = aws_security_group.emr_managed_agent.id
  description = "Combined TCP/UDP/ICMP"
}
...
//Combined TCP, UDP, ICMP SG rule sourcing EMR Managed Master SG in EMR Managed Agent SG
resource "aws_security_group_rule" "all_ports_for_master_sg_in_agent_sg" {
  from_port = 0
  to_port = 0
  protocol = "-1"
  security_group_id = aws_security_group.emr_managed_agent.id
  type = "ingress"
  source_security_group_id = aws_security_group.emr_managed_master.id
  description = "Combined TCP/UDP/ICMP"
}

//Combined TCP, UDP, ICMP SG rule sourcing EMR Managed Agent SG in EMR Managed Agent SG
resource "aws_security_group_rule" "all_ports_for_agent_sg_in_agent_sg" {
  from_port = 0
  to_port = 0
  protocol = "-1"
  security_group_id = aws_security_group.emr_managed_agent.id
...
//Egress SG rule for EMR Managed Agent SG
resource "aws_security_group_rule" "egress_for_agent_sg" {
  from_port = 0
  to_port = 0
  protocol = "-1"
  security_group_id = aws_security_group.emr_managed_agent.id
  ...
  description = "Egress rule for EMR Managed Agent SG"
}
...
//YARN NodeManager rule - Additional SG for EMR Agent with TAMR CIDRS
resource "aws_security_group_rule" "yarn_nodemanager_port_add_agent_sg_tamr_cidrs" {
  count = length(var.tamr_cidrs) > 0 ? 1 : 0
  from_port = 8042
  to_port = 8042
  protocol = "tcp"
  security_group_id = aws_security_group.emr_additional_agent.id
  type = "ingress"
  cidr_blocks = var.tamr_cidrs
  description = "YARN NodeManager - EMR Dashboard"
}

//YARN NodeManager rule - Additional SG for EMR Agent with TAMR SGs
resource "aws_security_group_rule" "yarn_nodemanager_port_add_agent_sg_tamr_sgs" {
  count = length(var.tamr_sgs) > 0 ? length(var.tamr_sgs) : 0
  from_port = 8042
  to_port = 8042
  protocol = "tcp"
...
//Hadoop HDFS DataNode 1 rule - Additional SG for EMR Agent with TAMR CIDRS
resource "aws_security_group_rule" "hdfs_datanode_1_port_add_agent_sg_tamr_cidrs" {
  count = length(var.tamr_cidrs) > 0 ? 1 : 0
  from_port = 50075
  to_port = 50075
  protocol = "tcp"
...
//Hadoop HDFS DataNode 1 rule - Additional SG for EMR Agent with TAMR SGs
resource "aws_security_group_rule" "hdfs_datanode_1_port_add_agentg_tamr_sgs" {
  count = length(var.tamr_sgs) > 0 ? length(var.tamr_sgs) : 0
  from_port = 50075
  to_port = 50075
  protocol = "tcp"
...
//Hadoop HDFS DataNode 2 rule - Additional SG for EMR Agent with TAMR CIDRS
resource "aws_security_group_rule" "hdfs_datanode_2_port_add_agent_sg_tamr_cidrs" {
  count = length(var.tamr_cidrs) > 0 ? 1 : 0
  from_port = 50010
  to_port = 50010
  protocol = "tcp"
...
//Hadoop HDFS DataNode 2 rule - Additional SG for EMR Agent with TAMR SGs
resource "aws_security_group_rule" "hdfs_datanode_2_port_add_agent_sg_tamr_sgs" {
  count = length(var.tamr_sgs) > 0 ? length(var.tamr_sgs) : 0
  from_port = 50010
  to_port = 50010
  protocol = "tcp"
...
//Region Server rule - Additional SG for EMR Agent with TAMR CIDRS
resource "aws_security_group_rule" "region_server_port_add_agent_sg_tamr_cidrs" {
  count = length(var.tamr_cidrs) > 0 ? 1 : 0
  from_port = 16020
  to_port = 16020
  protocol = "tcp"
...
//Region Server rule - Additional SG for EMR Agent with TAMR SGs
resource "aws_security_group_rule" "region_server_port_add_agent_sg_tamr_sgs" {
  count = length(var.tamr_sgs) > 0 ? length(var.tamr_sgs) : 0
  from_port = 16020
  to_port = 16020
  protocol = "tcp"
...
//Egress SG rule for EMR Agent Additional SG
resource "aws_security_group_rule" "egress_for_add_agent_sg" {
  from_port = 0
  to_port = 0
  protocol = "-1"
  security_group_id = aws_security_group.emr_additional_agent.id
  ...
  description = "Egress rule for EMR Agent Additional SG"
}
